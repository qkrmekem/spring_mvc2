...
//    }
    ...
    // bindingResult의 위치는 @ModelAttribute객체 바로 다음에 와야함
    // bindingResult가 기존의 errors 역할을 해줌
    // 또한 bindingResult는 자동으로 view로 넘어가기 때문에 따로 model에 담아주지 않아도 됨
//    @PostMapping("/add")
...
//    @PostMapping("/add")
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // 상품명이 없을 때
        if (!StringUtils.hasText(item.getItemName())) {
            // 에러메세지를 답은 code부분은 string배열이 들어가는데, 들어가는 순서대로 처음 값이 없으면 두번째 값을 반환한다.
...
//    @PostMapping("/add")
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // 검증 로직
        // 상품명이 없을 때
        // 아래의 if문과 똑같은 역할을 함(공백 같은 단순한 기능만 제공)
